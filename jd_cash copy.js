...
const inviteCodes = ['eU9YHpfKJohgkiyIvRVz']
if ($.isNode()) {
...
function exchangecash() {
  let cashurl = 'clientVersion=10.0.6&build=88852&client=android&d_brand=HUAWEI&d_model=COL-AL10&osVersion=10&screen=2190*1080&partner=huawei&oaid=00000000-0000-0000-0000-000000000000&openudid=bdc03a4fc9be38b2&eid=eidAdb328122ecs2/ZR+MYn6TnuBrU+XzgDVP/2Uf6gi2kcjT1yw+RVduUhe3Hh6PWdk4lBNQzYqodNGVx+mkdfB5RzmnLx7vYDhlJPYYKLWwTnuTqFO&sdkVersion=29&lang=zh_CN&uuid=bdc03a4fc9be38b2&aid=bdc03a4fc9be38b2&area=20_1715_43117_59198&networkType=wifi&wifiBssid=unknown&uts=0f31TVRjBSsqndu4%2FjgUPz6uymy50MQJJk%2B3SMkzXh%2BIqsOywVeoftC0M%2FH%2FGoI9jIrXP2WCoSotcFTw%2B94aZyLUptuY7%2FnGWHyRbU%2BP7%2BPW%2F8uKuS%2FtME2AVm5oC1T%2BUgrv6ROpKki3Ukmr8Er%2FJLdBpFEwYDadNaPo1MmFN%2BOEPXZKtxYSyidcXSP1Wxzf4nTLoIJF5Yma%2BQXFuJQZ%2Fg%3D%3D&uemps=0-0&harmonyOs=0&st=1625205333646&sign=9bd9c01f2ec7d9f54f1e73048905d372&sv=112'
  let body = {"amount":1000,"code":"031ES4000EVfZL1qZD100Y9ruQ3ES40n"};
  return new Promise((resolve) => {
    const options = {
      url: `${JD_API_HOST}?functionId=cash_wx_withdraw&${cashurl}`,
    ...
    $.post(options, async (err, resp, data) => {
      try {
        data = JSON.parse(data);
        console.log(`微信提现：${JSON.stringify(data)}\n`);
      } catch (e) {
        $.logErr(e, resp)
      } finally {
        resolve(data);
